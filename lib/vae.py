...
class VariationalAutoEncoder:
    ...
    def transform(self, data_x, weight_path):
        ...
        data_en = sess.run(z_mean, feed_dict={x: data_x})
        return data_en
    ...
    def run_all(self, data_x, lr, batch_size, epoch, print_step=100, x_valid=None):
        ...
        with tf.variable_scope("inference"):
            ...
            for i in range(1,len(self.dims)):
                key = 'W'+str(i+1)
                variable = tf.get_variable(key, initializer=tf.constant(self.weights[i]), dtype=tf.float32)
                rec[key] = variable
                key = 'b' + str(i+1)
                variable = tf.get_variable(key, initializer=tf.constant(self.biases[i]), dtype=tf.float32)
                rec[key] = variable

            rec['W_z_mean'] = tf.get_variable("W_z_mean", initializer=tf.constant(self.weights[-2]), dtype=tf.float32)
            rec['b_z_mean'] = tf.get_variable("b_z_mean", initializer=tf.constant(self.biases[-2]), dtype=tf.float32)
            rec['W_z_log_sigma'] = tf.get_variable("W_z_log_sigma", initializer=tf.constant(self.weights[-1]), dtype=tf.float32)
            rec['b_z_log_sigma'] = tf.get_variable("b_z_log_sigma", initializer=tf.constant(self.biases[-1]), dtype=tf.float32)

        with tf.variable_scope("generation"):
            gen = {}
            key_W = 'Wz'
            key_b = 'bz'
            gen[key_W] = tf.get_variable(key_W, initializer=tf.constant(self.de_weights[-1]), dtype=tf.float32)
            gen[key_b] = tf.get_variable(key_b, initializer=tf.constant(self.de_biases[-1]), dtype=tf.float32)
        ...
        z_mean = tf.matmul(h_prev, rec['W_z_mean']) + rec['b_z_mean']
        z_log_sigma_sq = tf.matmul(h_prev, rec['W_z_log_sigma']) + rec['b_z_log_sigma']

        eps = tf.random_normal((batch_size, self.n_z), 0, 1, 
            seed=0, dtype=tf.float32)
        z = z_mean + tf.sqrt(tf.maximum(tf.exp(z_log_sigma_sq), 1e-10)) * eps

        h_prev = self.activate(
            tf.matmul(z, gen['Wz']) + gen['bz'], self.activations[-1])
        for i in reversed(range(1,len(self.dims))):
            key_W = 'W'+str(i+1)
            key_b = 'b' + str(i+1)
            h_prev = self.activate(
    ...
    def run_latent(self, data_x, hidden_dim, batch_size, lr, epoch, print_step=100):
        ...
        rec = { 'W_z_mean': tf.get_variable("W_z_mean", [self.dims[-1], self.n_z], 
                ...
                'b_z_log_sigma': tf.get_variable("b_z_log_sigma", [self.n_z], 
                    initializer=tf.constant_initializer(0.0), dtype=tf.float32)}
        gen = {'Wz': tf.get_variable("Wz", [self.n_z, self.dims[-1]], 
                    initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32),
                'bz': tf.get_variable("bz", [self.dims[-1]], 
                    initializer=tf.constant_initializer(0.0), dtype=tf.float32)}
        x = tf.placeholder(dtype=tf.float32, shape=[None, input_dim], name='x')
        ...
        z = z_mean + tf.sqrt(tf.maximum(tf.exp(z_log_sigma_sq), 1e-10)) * eps
        x_recon = tf.matmul(z, gen['Wz']) + gen['bz']
        x_recon = tf.nn.sigmoid(x_recon, name='x_recon')
        gen_loss = -tf.reduce_mean(tf.reduce_sum(x * tf.log(tf.maximum(x_recon, 1e-10)) 
            + (1-x) * tf.log(tf.maximum(1 - x_recon, 1e-10)),1))
        latent_loss = 0.5 * tf.reduce_mean(tf.reduce_sum(tf.square(z_mean) + tf.exp(z_log_sigma_sq)
        ...
        self.de_weights.append(sess.run(gen['Wz']))
        self.de_biases.append(sess.run(gen['bz']))
